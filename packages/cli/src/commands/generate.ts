...
const MOCK_NAMES = [
  "Arpit Khandelwal", "Vikas Kumar", "Adya Archita", "John Doe", "Jane Smith",
  "Alice Johnson", "Bob Miller", "Sarah Connor", "Emily Watson", "David Davis",
  "Michael Chen", "Sophia Rodriguez", "Liam O'Connor", "Aria Patel", "Omar Farooq"
];

const MOCK_CATEGORIES = ["Electronics", "Office Supplies", "Apparel", "Home & Kitchen", "Fitness", "Books"];

const MOCK_LOG_SERVICES = ["auth-service", "db-driver", "gateway-api", "sync-worker", "indexing-engine"];
...
const MOCK_STATUSES = ["pending", "completed", "shipped", "cancelled", "active", "inactive", "todo", "in_progress", "done"];

const MOCK_PRODUCT_TITLES = [
  "Premium Mechanical Keyboard", "UltraWide 4K Monitor", "Wireless Ergonomic Mouse",
  "USB-C Multiport Adapter", "Noise Cancelling Headphones", "Portable SSD 1TB",
  "Dual-Screen Desk Stand", "Leather Executive Chair", "Smart Water Bottle"
];

const MOCK_TASK_TITLES = [
  "Implement Fractional Indexing", "Fix Typecheck Pipeline", "Design Neon SVG Charts",
  "Setup Offline AI Seeder", "Write Integration Tests", "Review Pull Requests",
  "Optimize Local DB Seeding", "Integrate Event Emitter Sync", "Refactor Auth0 Wrapper"
];
 ...
 */
export function generateLocalMockData(schema: Record<string, string>, count: number, promptContext: string): any[] {
  const isTask = promptContext.includes("task") || promptContext.includes("todo") || schema["title"]?.includes("task") || schema["title"]?.includes("todo");
  const isProduct = promptContext.includes("product") || promptContext.includes("item") || promptContext.includes("ecommerce");
  ...
  for (let i = 0; i < count; i++) {
    ...
    for (const [key, type] of Object.entries(schema)) {
      ...
      if (lowerKey === "id" || lowerKey === "_id" || lowerKey === "uuid") {
        record[key] = typeof randomUUID === "function"
          ? randomUUID()
      ...
      } else if (lowerKey.includes("price") || lowerKey.includes("amount") || lowerKey.includes("cost") || lowerKey.includes("total")) {
        record[key] = parseFloat((19.99 + (i * 12.5) + (i % 3) * 4.25).toFixed(2));
      } else if (lowerKey.includes("quantity") || lowerKey.includes("count") || lowerKey.includes("stock") || lowerKey.includes("age")) {
      ...
      } else if (lowerKey.includes("description") || lowerKey.includes("content") || lowerKey.includes("body")) {
      ...
      } else if (lowerKey.includes("avatar") || lowerKey.includes("image") || lowerKey.includes("url") || lowerKey.includes("photo")) {
        record[key] = `https://api.dicebear.com/7.x/adventurer/svg?seed=user_${i + 1}`;
      } else if (lowerKey.includes("date") || lowerKey.includes("created") || lowerKey.includes("updated") || lowerKey.includes("timestamp")) {
        const d = new Date();
        d.setDate(d.getDate() - i);
        record[key] = d.toISOString();
      } else if (lowerKey.includes("completed") || lowerKey.includes("active") || lowerKey.includes("admin") || lowerKey.includes("verified")) {
        record[key] = i % 2 === 0;
      } else {
...
function inferSchema(schemaPath: string): Record<string, string> {
    ...
    try {
      ...
      if (parsed.properties && typeof parsed.properties === "object") {
        for (const [key, val] of Object.entries(parsed.properties)) {
          const type = val && typeof val === "object" && "type" in val ? String(val.type) : "string";
          schema[key] = type;
        }
      } else {
  ...
  for (const line of lines) {
    ...
    if (tsMatch && tsMatch[1] && tsMatch[2]) {
      const type = tsMatch[2].toLowerCase().includes("number") ? "number" :
                   tsMatch[2].toLowerCase().includes("boolean") ? "boolean" : "string";
...
export async function generateCommand(options: GenerateOptions): Promise<void> {
  ...
  if (provider === "ollama") {
    try {
      console.log(chalk.yellow(`📡 Connecting to local Ollama instance at http://localhost:11434...`));
...
Your response MUST be a valid JSON array of objects. Do not wrap your response in markdown backticks, do not write explanations, and do not include additional text. Only return the JSON.`;
      ...
      console.log(chalk.green(`✅ Successfully generated ${records.length} records using local Ollama model ${model}.`));
    } catch (err: any) {
      console.warn(chalk.yellow(`⚠️  Ollama generation failed or unreachable: ${err.message}`));
      console.log(chalk.blue(`⚙️  Falling back to high-fidelity built-in offline Semantic Engine...`));
